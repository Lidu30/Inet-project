...
publicRouter.post("/timeslots/:id/reserve", async (req, res) => {
  ...
  const timeslot = await db.get(
    "SELECT * FROM timeslots WHERE timeslot_id = ? AND booked = 0",
    [id],
  );
  ...
  if (!timeslot) {
    return res
      .status(403)
      .json({ error: "Timeslot is already booked or rserved" });
  }
  ...
  if (!reserved) {
    return res
      .status(403)
      .json({ error: "Timeslot is already reserved or booked" });
  }
  ...
  setTimeout(async () => {
    const currentTimeslot = await db.get(
...
publicRouter.post("/timeslots/:id/cancel", async (req, res) => {
  ...
  if (!released) {
    return res
      .status(404)
      .json({ error: "Timeslot not found or not reserved" });
  }
...
privateRouter.delete("/timeslots/:id", requireAuth, async (req, res) => {
  ...
  if (!timeslot) {
    return res
      .status(404)
      .json({ error: "Timeslot not found or not owned by you" });
  }
